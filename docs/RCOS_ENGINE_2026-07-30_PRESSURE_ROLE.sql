...
/* ============================================================================
   إضافة 2026-07-30.5 — بوابة الطاولات وصفحة الصالة
   ----------------------------------------------------------------------------
   ع٥ — إسنادٌ يُعرض ولا يُنفَّذ: الإدارة تُسند طاولات بعينها في
        rko_table_duty، وبطاقة «طاولاتي اليوم» تقرأ منه مباشرةً، لكن
        rko__tbl_authz لم تكن تعرف هذا الجدول إطلاقاً. فتظهر الطاولة في
        قائمة الموظفة، وبمجرّد أن تضغطها يرفض الخادم: «هذه المنطقة غير
        مُسندة إليكِ». مقيسٌ على بيانات اليوم الحقيقية: سدره (٥ طاولات)
        وليان (٢) وهبه (٧) — ثلاثتهنّ كنّ يُرفَضن. والسبب المُركَّب أن
        شفتات اليوم أُنشئت بـ area_id فارغ، فلم يكن أي مسار آخر يُنجدهنّ.

   ع٦ — الرؤية كانت محكومةً ببوابة التعديل نفسها، فمن ليست منطقتها لا ترى
        الأرضية أصلاً. والصالة يجب أن تراها كل من تعمل فيها.

   الحلّ: نطاقٌ صريح لكل حالة داخل rko__tbl_authz:
     full         → منطقة الشفت، أو الكاش على مناطق الجلوس
     duty_tables  → طاولات أسندتها الإدارة بعينها (وكان هذا المسار مفقوداً)
     order_states → خط المطبخ: «قيد التجهيز» و«جاهز» فقط
     view         → كل من على الشفت: ترى الأرضية ولا تعدّلها
   و rko__tbl_set تُطيع النطاق، و rko__tbl_board تُعلنه للواجهة مع
   duty_tables فتُعطَّل الأزرار قبل أن يرفض الخادم.

   دليل القبول على بيانات اليوم (٢٠٢٦-٠٧-٣٠):
     سدره → duty_tables [9,10,4,6,8]  · can_edit = true
     ليان → duty_tables [3,2]          · can_edit = true
     هبه  → duty_tables [16..27] في الساحة الخارجية · can_edit = true
   ومن لا إسناد لها ولا منطقة: scope = view · can_edit = false، وتحديثها
   يُرفض بالرمز view_only، وتحديث طاولةٍ ليست من إسنادها يُرفض بـ
   not_my_table — كلٌّ بنصٍّ يقول لها ماذا تفعل بدلاً منه.
   ==========================================================================*/
